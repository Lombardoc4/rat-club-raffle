...
export const AdminRaffleForm = () => {
  ...
  const [raffle, setRaffle] = useState<UpdateRaffleInput | CreateRaffleInput>(
    emptyRaffle,
  );
        ...
        if (data.listRaffles.items.length > 0) {
          ...
          setRaffle({
            ...
            drawing_date: drawing_date
              ? moment(drawing_date).format('YYYY-MM-DDTHH:mm:ss')
              : '',
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const formattedRaffle = {
      ...
      drawing_date: raffle.drawing_date
        ? moment(raffle.drawing_date).toISOString()
        : null,
    };
  ...
  return (
    <div className='mx-auto max-w-96'>
      <h1 className='mb-2 text-4xl'>Raffle Form</h1>
      {stage === 1 && (
