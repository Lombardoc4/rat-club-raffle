...
export const validateDrawingDate = (drawing: string, start?: string, end?: string) => {
    if (!start || !end) {
        return false;
    }
    ...
    return moment(drawingDate).isAfter(moment(startDate)) && moment(drawingDate).isAfter(moment(endDate));
};